...
def most_viewed(username):
    r = redis.Redis()
    user_score = r.zincrby("userclick",username, 1)
    return user_score

"""API for Most Top PER PAGE Viewed User"""
@app.route('/api/mostviewed/<perPage>', methods=['GET'])
def most_viewed_users(perPage):
    r = redis.Redis()
    """Incrementing User score after Hits"""
#    print "Printitng Incremented score of", username, ":", user_score
    """Display user score and usernames"""
    all_userscores = {}
    all_userscores=r.zrevrange("userclick",0,perPage,"withscores")
    all_users = []
    for key, value in all_userscores:
        user = {"username": key,"score": int(value)}
        all_users.append("All users:")
        all_users.append(user)
    print "This is score with username", all_users
    #print (message)
    #json = [sorted(dict(r).items(),reverse=True) for r in message]
    return jsonify({'all_userscores': all_users}), 200


def most_followed_user(whomUserName):
    r = redis.Redis()
    f_user_score = r.zincrby("fuserclick",whomUserName, 1)
    return f_user_score

def ulead_timeline(whomUserName):
    r = redis.Redis()
    u_user_score = r.zincrby("fuserclick",whomUserName, -1)
    return u_user_score

"""API for Most Top PER PAGE Followed User"""

@app.route('/api/mostfollowed/<perPage>')
def most_followed_users(perPage):
    r = redis.Redis()
    """Incrementing User score after follow Hits"""
    """follow Display user score and usernames"""
    f_all_userscores = {}
    f_all_userscores=r.zrevrange("fuserclick",0,perPage,"withscores")
    f_all_users = []
    for key, value in f_all_userscores:
        fuser = {"whomUserName": key,"score": int(value)}
        f_all_users.append("user")
        f_all_users.append(fuser)
    print "This is score with username", f_all_users
    #print (message)
    #json = [sorted(dict(r).items(),reverse=True) for r in message]
    return jsonify({'f_all_userscores': f_all_users}), 200
...
def follow(whomUserName,whoUserName):
    ...
    if not followed:
        ...
        most_followed_user(whomUserName)
        #most_followed_users()
