...
@app.route('/mostviewedscore')
def most_viewed_score():
    response = requests.get(API_BASE_URL + '/api/mostviewed/' + PER_PAGE)
    all_messages = json.loads(response.text)
    print all_messages, "Most Viewd Users Scores"
    print all_messages.get('all_userscores',[])
    return render_template('lead.html', all_userscores = all_messages.get('all_userscores',[]))

@app.route('/mostfollowedscore')
def most_followed_score():
    response = requests.get(API_BASE_URL + '/api/mostfollowed/' + PER_PAGE)
    all_messages = json.loads(response.text)
    print all_messages, "Most followed users Scores"
    print all_messages.get('f_all_userscores',[])
    return render_template('hits.html', f_all_userscores = all_messages.get('f_all_userscores',[]))
